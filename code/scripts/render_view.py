...
def get_kl_data():
    ...
    df_chain = pd.read_csv(f"{data_path}_chain_{base_perp}.csv",
                           index_col='perplexity')
    df_normal = pd.read_csv(f"{data_path}_normal_{base_perp}.csv",
                            index_col='perplexity')
    df_compare = pd.read_csv(f"{data_path}_{base_perp}.csv",
                             index_col='perplexity')
...
def gen_normal_figures():
    tbody = '\n'.join([
        ...
        TR.format('\n'.join([
            TD.format(gen_fig('normal', base_perp)),
            TD.format('tSNE chain')
        ])),
        TR.format(TDP.format('Compare tSNE chain & normal')),
        TR.format('\n'.join([
            TD.format(gen_fig('normal', base_perp)),
            TD.format('tSNE normal')
        ])),
        TR.format(TDP.format('(Q_base)')),
    ])
...
def gen_chain_figures():
    ...
    row_kl_chains = [TDP.format('KL[Q_base||Q_chain] = {:.3f}'.format(
        kl_data['kl_chain'].get(p, -1)
    )) for p in perp_range]

    row_kl_normals = [TDP.format('KL[Q_base||Q_normal] = {:.3f}'.format(
        kl_data['kl_normal'].get(p, -1)
    )) for p in perp_range]

    row_kl_chain_normal = [TDP.format('KL[Q_chain||Q_normal] = {:.3f}'.format(
